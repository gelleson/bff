...
export class TransactionService {
  ...
  public getTransactionsByDate(date: string) {
    return this.http.get<PaginationObject<TransactionObject>>(`${this.baseUrl}/transactions?date=${date}`);
  }

  public getTransactions(date: Date) {
    const formattedDate = date.toISOString().slice(0, 10);

    return this.http.get<PaginationObject<TransactionObject>>(`${this.baseUrl}/transactions?date=${formattedDate}`);
  }
